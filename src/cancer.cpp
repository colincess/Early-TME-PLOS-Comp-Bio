...
extern int totalCancerCells;

Cancer::Cancer(std::array<int, 2> loc, int index, double IL4prod, double IFNGprod, int initial){
    location = loc;
    idx = index;
    state = "alive";

    // stem cells proliferate slower than progenitor cells
    div = 20;//paramsLHS[3];//30;
...
void Cancer::proliferate(CellGrids &cg, std::vector<Cancer> &cc_list){
    ...
    cg.allCells[ni][nj] = 1 ;
    cg.ccg[ni][nj] = 1;
    cc_list.push_back(Cancer({ni,nj}, cc_list.size(), kIL4c, kIFNGc, 0));
